...
# 构建 Erf ONNX 模型
# ======================
def build_erf_onnx_model(input_shape, save_path):
    """
    Erf (Gauss Error Function) 函数
    计算高斯误差函数
    ...
    print(f"[INFO] 成功导出 Erf ONNX 单算子模型: {save_path}")
...
# ======================
def main():
    parser = argparse.ArgumentParser(description="构建 Erf ONNX 单算子模型")
    ...
    dir_name = f"erf_onnx_input_{input_shape_str}"
    os.makedirs(dir_name, exist_ok=True)
    model_path = os.path.join(dir_name, f"onnx_input_{input_shape_str}_erf.onnx")

    # ===== Step 3: 构建模型 =====
    build_erf_onnx_model(input_shape, model_path)
